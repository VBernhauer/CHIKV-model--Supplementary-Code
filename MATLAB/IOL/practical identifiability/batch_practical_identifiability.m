function [] = batch_practical_identifiability(nprocs)
    %%% command: batch_practical_identifiability(1)
    ...
    timeout = 86400;
    nrounds = length(parameters); % number of runs
    ...
    for iproc = 1:nprocs
        ...
        task{iproc} = createTask(j, @practical_identifiability, 1, {nstart(iproc), nend(iproc), parameters});
        fprintf('done!\n');
    end
